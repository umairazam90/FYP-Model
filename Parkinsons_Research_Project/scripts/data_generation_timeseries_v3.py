...
def generate_signal(base_freq, length, noise_level, drift, jitter):
    ...
    freq_with_jitter = base_freq * (1 + np.random.normal(0, 0.1) * jitter) 
...
def generate_pd_voice_signal(length=128):
    ...
    return generate_signal(base_freq=10.2, length=length, noise_level=0.4, drift=0.3, jitter=0.5)

def generate_control_voice_signal(length=128):
    '''Simulates a healthy voice signal (also very noisy, high overlap)'''
    return generate_signal(base_freq=10.0, length=length, noise_level=0.3, drift=0.2, jitter=0.3)

def generate_pd_gait_signal(length=256):
    '''Simulates a PD gait signal (still challenging)'''
    t = np.linspace(0, 8, length)
    freq = 1.8 + np.random.normal(0, 0.4) 
    signal = np.sin(t * freq * np.pi * 2)**2 
    noise = np.random.randn(length) * 0.20 
    try:
        hesitation_start = np.random.randint(50, 150)
        hesitation_len = np.random.randint(10, 30)
        if hesitation_start + hesitation_len < length:
             signal[hesitation_start : hesitation_start + hesitation_len] *= np.random.uniform(0.1, 0.4)
    except ValueError as e:
        print(f"Warning: Hesitation generation failed - {e}")
    return signal + noise

def generate_control_gait_signal(length=256):
    '''Simulates a healthy, rhythmic gait signal (also noisier)'''
    t = np.linspace(0, 8, length)
    freq = 2.0 + np.random.normal(0, 0.1) 
    signal = np.sin(t * freq * np.pi * 2)**2 
    noise = np.random.randn(length) * 0.10 
    return signal + noise

#
# >>>>>>>>>>>>>> THIS IS THE FIX: Renamed class to _V3
#
class UltraRealisticDataGenerator_TS_V3: 
    ...
    def generate_ts_voice_data(self, n_controls=500, n_parkinsons=500):
        print(f"🎤 Generating {n_controls+n_parkinsons} TS voice subjects (V3)...")
        n_subjects = n_controls + n_parkinsons
        voice_data = []
        for subject_id in tqdm(range(1, n_subjects + 1), desc="Generating Voice Subjects (V3)"):
        ...
        voice_df = pd.DataFrame(voice_data); print(f"✅ Generated TS voice data: {voice_df.shape}")
        return voice_df

    def generate_ts_gait_data(self, n_controls=500, n_parkinsons=500):
        print(f"🚶 Generating {n_controls+n_parkinsons} TS gait subjects (V3)...")
        n_subjects = n_controls + n_parkinsons
        gait_data = []
        for i in tqdm(range(n_subjects), desc="Generating Gait Subjects (V3)"):
        ...
        gait_df = pd.DataFrame(gait_data); print(f"✅ Generated TS gait data: {gait_df.shape}")
        return gait_df

#
# >>>>>>>>>>>>>> THIS IS THE FIX: Renamed class to _V3
#
class RealisticFederatedDataset_TS_V3: 
    ...
    def create_realistic_pairs(self, voice_df, gait_df, voice_features, gait_features):
        print("Creating realistic multimodal TS pairs (V3)...")
        subject_voice = voice_df.copy(); gait_processed = gait_df.copy() 
        ...
        for i in tqdm(range(n_pairs), desc="Generating TS Pairs (V3)"):
            ...
            if np.random.random() < self.label_noise:
                ...
                gait_row = gait_parkinsons.sample(1).iloc[0] if voice_label == 0 else gait_controls.sample(1).iloc[0]
            else:
                pair_type = "MATCHED"
                gait_row = gait_controls.sample(1).iloc[0] if voice_label == 0 else gait_parkinsons.sample(1).iloc[0]
