...
#
# >>>>>>>>>>>>>> THIS IS THE FIX:
# The parameters for PD and Control are now MUCH closer to create overlap
#
def generate_pd_voice_signal(length=128):
    '''Simulates a PD voice signal (slightly noisy)'''
    # Reduced jitter/noise from original
    return generate_signal(base_freq=10, length=length, noise_level=0.25, drift=0.3, jitter=0.3)

def generate_control_voice_signal(length=128):
    '''Simulates a healthy voice signal (also slightly noisy)'''
    # Increased jitter/noise from original
    return generate_signal(base_freq=10, length=length, noise_level=0.15, drift=0.15, jitter=0.15)
#
# <<<<<<<<<<<<<< END OF FIX
#
...
def generate_control_gait_signal(length=256):
    '''Simulates a healthy, rhythmic gait signal (still challenging)'''
    t = np.linspace(0, 8, length)
...
class UltraRealisticDataGenerator_TS_V2: # Renamed class
    ...
    def generate_ts_voice_data(self, n_controls=500, n_parkinsons=500):
        print(f"🎤 Generating {n_controls+n_parkinsons} TS voice subjects (V2)...")
        n_subjects = n_controls + n_parkinsons
        voice_data = []
        for subject_id in tqdm(range(1, n_subjects + 1), desc="Generating Voice Subjects (V2)"):
            is_parkinson = 1 if subject_id > n_controls else 0
            if is_parkinson:
                signal = generate_pd_voice_signal(self.voice_length) # Uses new function
            else:
                signal = generate_control_voice_signal(self.voice_length) # Uses new function
    ...
    def generate_ts_gait_data(self, n_controls=500, n_parkinsons=500):
        print(f"🚶 Generating {n_controls+n_parkinsons} TS gait subjects (V2)...")
        n_subjects = n_controls + n_parkinsons
        gait_data = []
        for i in tqdm(range(n_subjects), desc="Generating Gait Subjects (V2)"):
...
class RealisticFederatedDataset_TS_V2: # Renamed class
    ...
    def create_realistic_pairs(self, voice_df, gait_df, voice_features, gait_features):
        print("Creating realistic multimodal TS pairs (V2)...")
        ...
        for i in tqdm(range(n_pairs), desc="Generating TS Pairs (V2)"):
